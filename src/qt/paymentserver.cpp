...
const int ERA_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ERA_IPC_PREFIX("era:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        socket->waitForBytesWritten(ERA_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click era: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start era: click-to-pay handler");
    else
