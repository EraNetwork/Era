...
{
    return QCoreApplication::translate("era-core", psz).toStdString();
}
...
int main(int argc, char *argv[])
{
    ...
    // Load e.g. era_de.qm (shortcut "de" needs to be defined in era.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        app.installTranslator(&translatorBase);

    // Load e.g. era_de_DE.qm (shortcut "de_DE" needs to be defined in era.qrc)
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit Era-Qt here
