...
void EraAmountField::clear()
{
    amount->clear();
    unit->setCurrentIndex(0);
}

bool EraAmountField::validate()
{
    bool valid = true;
    if (amount->value() == 0.0)
        valid = false;
    if (valid && !EraUnits::parse(currentUnit, text(), 0))
        valid = false;
...
void EraAmountField::setValid(bool valid)
{
    if (valid)
...
QString EraAmountField::text() const
{
...
void EraAmountField::setValue(qint64 value)
{
    setText(EraUnits::format(currentUnit, value));
}

void EraAmountField::unitChanged(int idx)
{
    // Use description tooltip for current unit for the combobox
    unit->setToolTip(unit->itemData(idx, Qt::ToolTipRole).toString());

    // Determine new unit ID
    int newUnit = unit->itemData(idx, EraUnits::UnitRole).toInt();
...
void EraAmountField::setDisplayUnit(int newUnit)
{
