...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(EraUnits::ERA)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseBlackTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = EraUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    ui->labelBalance->setText(EraUnits::formatWithUnit(unit, balance));
    ui->labelStake->setText(EraUnits::formatWithUnit(unit, stake));
    ui->labelUnconfirmed->setText(EraUnits::formatWithUnit(unit, unconfirmedBalance));
    ui->labelImmature->setText(EraUnits::formatWithUnit(unit, immatureBalance));
    ui->labelTotal->setText(EraUnits::formatWithUnit(unit, balance + stake + unconfirmedBalance + immatureBalance));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("ERA")
    updateDisplayUnit();
}
